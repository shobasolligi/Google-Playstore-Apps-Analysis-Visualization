...
SELECT REPLACE(App, '?', '') App
FROM dbo.new_app1$
WHERE App LIKE '?%'

ALTER TABLE dbo.new_app1$
ADD App1 NVARCHAR(255) NULL

UPDATE dbo.new_app1$
SET App1 = REPLACE(App, '?', '')
FROM dbo.new_app1$
WHERE App LIKE '?%'
...
SELECT REPLACE([Last Updated],'-','/') [Last Updated]
FROM dbo.new_app1$

UPDATE dbo.new_app1$
SET Last_Updated = REPLACE([Last Updated], '-','/')
FROM dbo.new_app1$
...
ALTER TABLE dbo.new_app1$
ALTER COLUMN Last_Updated NVARCHAR(255) NULL
